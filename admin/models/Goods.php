...
class Goods extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['store_id', 'cat_id', 'status', 'addtime', 'is_delete', 'sort', 'virtual_sales', 'individual_share', 'freight', 'use_attr', 'share_type', 'quick_purchase', 'hot_cakes', 'mch_id', 'goods_num', 'member_discount', 'mch_sort', 'show_integral'], 'integer'],
        ...
        foreach ($attr_rows as $i => $attr_row) {
            ...
            if (!array_diff($attr_id_list, $key)) {
                if (!$attr_rows[$i]['price']) {
                    $attr_rows[$i]['price'] = strval($this->price);
                } else{
                    $attr_rows[$i]['price'] = strval($attr_rows[$i]['price']);
                }
...
//            }
    ...
    public function getMchInfo($mch_id)
    {
        return Mch::find()->where(['id' => $mch_id])->one();
    }
