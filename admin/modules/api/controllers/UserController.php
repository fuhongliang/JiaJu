...
use app\modules\api\models\CreateUserMessageForm;
...
use app\modules\api\models\MchFavoriteAddForm;
use app\modules\api\models\MchFavoriteRemoveForm;
...
//            'parent' => $share ? $share->name : ($parent ? $parent->nickname : '总店'),
            ...
            'is_share'          => !empty($is_share)?true:false
        ];
        ...
        $share_setting['work_type'] = $work_type;
     ...
     */
    public function actionMchFavoriteAdd()
    {
        $form = new MchFavoriteAddForm();
        $form->attributes = \Yii::$app->request->post();
        $form->store_id = $this->store->id;
        $form->user_id = \Yii::$app->user->id;
        return new BaseApiResponse($form->save());
     ...
     * @return BaseApiResponse
     */
    public function actionMchFavoriteRemove()
    {
        $form = new MchFavoriteRemoveForm();
        $form->attributes = \Yii::$app->request->post();
        $form->store_id = $this->store->id;
        $form->user_id = \Yii::$app->user->id;
        return new BaseApiResponse($form->save());
    ...
    {
        $form = new CreateUserMessageForm();
        $form->store_id = $this->store->id;
        $form->user_id = \Yii::$app->user->id;
