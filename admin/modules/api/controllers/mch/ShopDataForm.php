...
class ShopDataForm extends Model
{
    ...
    public function search()
    {
        ...
        $recommend_list = (new GoodsService())->getMchRecommendGoods($this->mch_id, 3);
        ...
        $favorite = MchFavorite::findOne(['user_id' => $this->user_id, 'mch_id' => $this->mch_id, 'is_delete' => 0]);
    ...
    public function getShop()
    {
        ...
        $mch_desc = empty($mch->mch_desc) ? '商家暂时还没完善简介^_^...' : $mch->mch_desc;
        $shop = [
            'id' => $mch->id,
            'name' => $mch->name,
            'mch_desc' => $mch_desc,
            'header_color' => $mch->header_color,
            'mch_color' => $mch->mch_color,
    ...
    public function getHotList()
    {
        ...
        $list = $query->select('g.*,od.sale_num')->orderBy('g.mch_sort,g.addtime DESC')->asArray()->all();
        ...
        $list = $query->select('g.*,od.sale_num')->orderBy('od.sale_num DESC,g.mch_sort,g.addtime DESC')->asArray()->all();
        if (is_array($list) && count($list))
            return $list;
        return [];
    }
    ...
    public function getGoodsList()
    {
        ...
        $pagination = new Pagination(['totalCount' => $count, 'page' => $this->page - 1, 'pageSize' => $this->limit]);
        $query->limit($pagination->limit)->offset($pagination->offset);
        $list = $query->select('g.*')->asArray()->all();
        if (is_array($list) && count($list))
            return $list;
        return [];
    }
    ...
    public function getNewList()
    {
        ...
        $query->limit($pagination->limit)->offset($pagination->offset);
        $list = $query->select('g.*')->orderBy('g.addtime DESC')->asArray()->all();
        $new_list = [];
