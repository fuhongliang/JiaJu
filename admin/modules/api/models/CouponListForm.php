    ...
    public function getList()
    {
        ...
        if ($this->type == 2) {
            $query = Coupon::find()->alias('c')->where([
                'c.is_delete' => 0, 'c.user_type' => 1, 'c.store_id' => $this->store_id
            ]);
        ...
        foreach($coupon_list as $index=>$value){
            ...
            //expire_type 1=领取后N天过期，2=指定有效期
            if ($value['expire_type'] == 1) {
                $coupon_list[$index]['show_time'] = '领取后'.$value['expire_day'].'天有效';
            } else {
            ...
            if($value['appoint_type'] == 1){
                ...
                $coupon_list[$index]['content'] = "指定分类使用";
