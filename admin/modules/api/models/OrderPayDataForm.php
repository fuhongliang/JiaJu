...
use app\services\AppWxPayService;
...
class OrderPayDataForm extends Model
{
    ...
    public function search()
    {
        ...
        if ($this->order_id) {//单个订单付款
            ...
            if ($this->pay_type == 'WECHAT_PAY') {
                ...
                if (!empty($this->app_key) && $this->app_key == 'app') {
                    //app 支付
                    $pay_data = [];
                    $app_wx_pay_service = new AppWxPayService();
                    $pay_data['appid'] = $app_wx_pay_service->appid;
                    $pay_data['partnerid'] = $app_wx_pay_service->mch_id;
                    $pay_data['prepayid'] = $res['prepay_id'];
                    $pay_data['package'] = 'Sign=WXPay';
                    $pay_data['noncestr'] =  md5(uniqid());
                    $pay_data['timestamp'] = time();
                    $pay_data['sign'] = $app_wx_pay_service->makeSign($pay_data);
                } else {
    ...
    private function unifiedOrder($goods_names)
    {
        if (!empty($this->app_key) && $this->app_key == 'app') {
            $app_wx_pay_service = new AppWxPayService();
            $res = $app_wx_pay_service->unifiedOrder([
                'body' => $goods_names,
    ...
    private function unifiedUnionOrder($order_list, $total_pay_price)
    {
        ...
        if (!empty($this->app_key) && $this->app_key == 'app') {
            $app_wx_pay_service = new AppWxPayService();
            $res = $app_wx_pay_service->unifiedOrder($data);
        } else {
