...
class OrderSubmitForm extends OrderData
{
    ...
    {
        return [
            [['cart_id_list', 'goods_info', 'content', 'address_name', 'address_mobile', 'cart_list', 'mch_list','other_mobile'], 'string'],
            ...
            [['address_mobile','other_mobile'],'match','pattern' =>Model::MOBILE_PATTERN , 'message'=>'手机号错误']
        ];
    }
    ...
    public function save()
    {
        ...
        $this->store = $store;
        if (!empty($this->cart_id_list) || !empty($this->mch_list)) {//购物车订单（新增多商户的商品）
        ...
        if (!empty($goods_list)) {//平台自营下的单子
            ...
            $order->other_mobile = $this->other_mobile;
            if ($this->payment == 2) {
                $order->pay_type = 2;
                $order->is_pay = 0;
            }
        ...
        if (!empty($data['mch_list'])) {//入驻商商品下的单子
            foreach ($data['mch_list'] as $mch) {
                //获取商家极光注册id
                $mch_info = Mch::findOne(['id' => $mch['mch_id']]);
                $jpush_registration_id = empty($mch_info['jpush_registration_id']) ? '' : $mch_info['jpush_registration_id'];
                $pushService = new PushService();
                $pushService->newOrder($jpush_registration_id);
    ...
    private function getGoodsListByGoodsInfo($goods_info)
    {
        ...
        \Yii::$app->db->createCommand()->insert('{{%api_log}}', [
            'param' => json_encode($goods_info),
            'created_at' => time(),
            'access_date' => date('Y-m-d H:i:s'),
        ])->execute();

        \Yii::$app->db->createCommand()->insert('{{%api_log}}', [
            'param' => json_encode($goods),
            'created_at' => time(),
            'access_date' => date('Y-m-d H:i:s'),
        ])->execute();
