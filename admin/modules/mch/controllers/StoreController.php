...
use app\modules\mch\models\GoodsBannerForm;
...
class StoreController extends Controller
{
     ...
     * 商品列表幻灯片
     */
    public function actionGoodsSlide()
    {
        $store = $this->store->id;
        $form = new GoodsBannerForm();
        $arr = $form->getList($store);
        return $this->render('goods-slide', [
            'list' => $arr[0],
            'pagination' => $arr[1],
        ]);
    }
     ...
     * 幻灯片添加修改
     */
    public function actionGoodsSlideEdit($id = 0)
    {
        $banner = GoodsBanner::findOne(['id' => $id]);
        if (!$banner) {
            $banner = new GoodsBanner();
        }
        $form = new GoodsBannerForm();
        if (\Yii::$app->request->isPost) {
            $model = \Yii::$app->request->post('model');
        ...
        return $this->render('goods-slide-edit', [
            'list' => $banner,
        ]);
    }
     ...
     * @param int $id
     */
    public function actionGoodsSlideDel($id = 0)
    {
        $dishes = GoodsBanner::findOne(['id' => $id, 'is_delete' => 0]);
        if (!$dishes) {
            return [
                'code' => 1,
                'msg' => '幻灯片不存在或已经删除',
    ...
    {
        $cat_list = Cat::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->orderBy('sort,addtime DESC')->asArray()->all();
        $data = $this->getTree($cat_list);
...
//                Option::set('home_page_data',json_encode($update_list,JSON_UNESCAPED_UNICODE),$this->store->id,'app');
    ...
    public function actionUploadFileList($type = 'image', $page = 1, $dataType = 'json', $group_id = -1)
    {
        ...
        $query = UploadFile::find()
            ->where(['store_id' => $this->store->id,'mch_id'=>$this->mch_id, 'is_delete' => 0, 'type' => $type]);
