...
class MchAddForm extends MchModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['user_id', 'realname', 'tel', 'name', 'province_id', 'city_id', 'district_id', 'address', 'mch_common_cat_id', 'service_tel', 'transfer_rate','password', 'username'], 'required'],
...
//        }
        ...
        //验证用户名
        $check = Mch::find()->where(['username'=>$this->username])->one();
        if (!empty($check)) {
            return [
                'code' => 1,
                'msg' => '用户名已存在',
        ...
        $mch->access_token = \Yii::$app->security->generateRandomString();
        $mch->auth_key = \Yii::$app->security->generateRandomString();
        $mch->password = \Yii::$app->security->generatePasswordHash($this->password, 5);
