                ...
                <div class="col-sm-6">
                    ...
                    <?php if (is_array($miaosha->open_time) && count($miaosha->open_time)):foreach ($miaosha->open_time as $i): $i = intval($i) ?>
                        <div class="custom-control custom-checkbox custom-control-inline">
                            <input name="open_time[]" type="checkbox" id="open_time_<?= $i ?>"
                                   class="custom-control-input open-time-checkbox"
                                   value="<?= $i ?>">
                            <label class="custom-control-label" for="open_time_<?= $i ?>">
                                <?= $i < 10 ? '0' . $i : $i ?>:00~<?= $i < 10 ? '0' . $i : $i ?>:59
                            </label>
                        </div>
                    <?php endforeach; else: ?>
...
<script>
    ...
    $(document).on("click", ".miaosha-submit-btn", function () {
        if (!app.goods || app.open_time.length == 0 || app.open_date.length == 0) {
            ...
            console.log('open_time-------', app.open_time);
            console.log('open_date-------', app.open_date);
            $.myToast({
                content: "请完善秒杀商品信息",
            });
            return;
